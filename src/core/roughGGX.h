...
struct RayInfo
{
	...
	void updateDirection(const Vector3f& w, const float alpha_x, const float alpha_y)
	{
		this->w = w;
		theta = acosf(w.z);
		cosTheta = w.z;
...
inline float D_ggx(const Vector3f& wm, const float alpha_x, const float alpha_y) {
	if( wm.z <= 0.0f)
		return 0.0f;
...
inline Vector3f sampleP22_11(const float theta_i, const float U, const float U_2, const float alpha_x, const float alpha_y)
{
	Vector3f slope(0, 0, 0);
...
inline Vector3f sampleVNDF(const Vector3f& wi, const float alpha_x, const float alpha_y)
{
